...
import { AuthError, homeRouteFor } from "@/lib/auth";
    ...
    <div className="flex min-h-screen items-center justify-center bg-canvas px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-border bg-surface p-6 shadow-pop">
          <div className="mb-5 flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-ink-faint">Facility Partner Portal</p>
            </div>
          </div>

          {authRequired === false && (
            <p className="mb-4 rounded-lg border border-border bg-surface-2 px-3 py-2 text-xs text-ink-muted">
              Sign-in is currently open on this environment — you can enter the portal directly.
            </p>
          )}
          ...
          <form onSubmit={onSubmit} className="space-y-3.5">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-rose text-sm font-semibold text-rose-contrast transition-colors hover:bg-rose-strong disabled:opacity-60"
            >
        ...
        <p className="mt-6 text-center text-xxs text-ink-faint">
          LaundryKhalas partner access. Restricted and audited.
        </p>
      </div>
    </div>
  );
