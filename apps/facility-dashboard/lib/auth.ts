/**
 * Auth network calls + pure route-access helpers for the facility dashboard.
 *
 * The dashboard authenticates against the FastAPI backend (never Supabase
 * directly). When the backend runs with REQUIRE_AUTH=false (local dev)
 * `/api/auth/config` reports `auth_required: false` and the UI skips the login
 * wall entirely.
 */
import {
  type AuthUser,
...
/** Every facility page lives under "/". */
export function homeRouteFor(_role: UserRole | null): string {
  return "/";
}
